"""Config IO + selection helpers for the NCI_GDC_TCGA_Splice library."""
from __future__ import annotations
import csv, os
from pathlib import Path
def repo_root()->Path: return Path(__file__).resolve().parents[3]
def library_root()->Path: return repo_root()/"geneset-extractor-dev"/"NCI_GDC_TCGA_Splice"
def config_root()->Path:
    o=os.environ.get("TCGA_SPLICE_CONFIG_ROOT","").strip(); return Path(o).expanduser().resolve() if o else library_root()/"config"
def default_out_root()->Path:
    o=os.environ.get("TCGA_SPLICE_OUT_ROOT","").strip(); return Path(o).expanduser().resolve() if o else Path.cwd()/"tcga_splice_outputs"
def default_model_list_path()->Path:
    o=os.environ.get("TCGA_SPLICE_MODEL_LIST","").strip(); return Path(o).expanduser().resolve() if o else config_root()/"model_list.tsv"
def default_tumor_type_list_path()->Path:
    o=os.environ.get("TCGA_SPLICE_TUMOR_TYPE_LIST","").strip(); return Path(o).expanduser().resolve() if o else config_root()/"tumor_type_list.tsv"
def default_model_manifest_path()->Path:
    o=os.environ.get("TCGA_SPLICE_MODEL_MANIFEST","").strip(); return Path(o).expanduser().resolve() if o else config_root()/"model_manifest.tsv"
def read_tsv(path):
    with Path(path).open("r",encoding="utf-8",newline="") as h: return list(csv.DictReader(h,delimiter="\t"))
...
def resolve_requested_ids(*,csv_text,file_path,rows,key_field):
    ...
    order={i:n for n,i in enumerate(all_ids)}; return sorted(set(requested),key=lambda i: order[i])
def row_map(rows,key): return {str(r.get(key,"")).strip():r for r in rows if str(r.get(key,"")).strip()}
def model_group_for(model_id):
    if model_id.startswith("SP"): return "splice_tumor_vs_normal"
    raise SystemExit(f"Unsupported model prefix for {model_id}")
def relative_or_absolute_path(t): p=Path(t); return p if p.is_absolute() else repo_root()/p
