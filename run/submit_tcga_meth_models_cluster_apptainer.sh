#!/usr/bin/env bash
set -euo pipefail
# Submit TCGA methylation tumor-vs-normal models as a qsub array (inside Apptainer).
# Mirrors the RNAseq/CNV submit scripts. One family (MD*); only projects with matched normals.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
REPO_ROOT="${REPO_ROOT:-${DEFAULT_REPO_ROOT}}"
SELF_PATH="${REPO_ROOT}/geneset-extractor-dev/run/submit_tcga_meth_models_cluster_apptainer.sh"
...
JOB_NAME="${TCGA_METH_JOB_NAME:-tcga_meth_models_apptainer}"; QSUB_BIN="${QSUB_BIN:-qsub}"
...
run_task(){
  ...
  bind_csv="$( { printf '%s\n' "${REPO_ROOT}" "${DIG_DIR}" "${OUT_ROOT}"; append_bind_path "${CONFIG_ROOT}"; append_bind_path "${BETA_MATRIX_TSV}"; append_bind_path "${SAMPLE_METADATA_TSV}"; append_bind_path "${GTF}"; append_bind_path "${PROBE_MANIFEST_TSV}"; } | awk 'NF && !seen[$0]++' | paste -sd, - )"
  if [[ "${MODE}" == "write_model_only" ]]; then
    inner="'${APPTAINER_PYTHON_BIN}' '${REPO_ROOT}/geneset-extractor-dev/NCI_GDC_TCGA_Methylation/src/run_methylation_diff_model.py' --model_id '${model}' --tumor_type_id '${tt}' --tumor_type_label '${label}' --project_id '${proj}' --run_root '${OUT_ROOT}/genesets/${tt}/models' --dig_dir '${DIG_DIR}' --model_manifest '${MODEL_MANIFEST}' --write_model_only"
  else
    inner="'${APPTAINER_PYTHON_BIN}' '${REPO_ROOT}/geneset-extractor-dev/NCI_GDC_TCGA_Methylation/src/build_tcga_meth_genesets.py' --tumor_types '${tt}' --models '${model}' --beta_matrix_tsv '${BETA_MATRIX_TSV}' --sample_metadata_tsv '${SAMPLE_METADATA_TSV}' --gtf '${GTF}' --dig_dir '${DIG_DIR}' --out_root '${OUT_ROOT}' --model_manifest '${MODEL_MANIFEST}' --model_list '${MODEL_LIST}' --tumor_type_list '${TUMOR_TYPE_LIST}'"
    [[ -n "${PROBE_MANIFEST_TSV}" ]] && inner+=" --probe_manifest_tsv '${PROBE_MANIFEST_TSV}'"
