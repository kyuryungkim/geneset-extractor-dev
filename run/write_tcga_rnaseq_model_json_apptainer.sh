...
# Write only the geneset.model.json sidecar for a TCGA RNA-seq model, inside Apptainer.
# Mirrors run/write_gtex_model_json_apptainer.sh.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --run_root|--counts_tsv|--sample_metadata_tsv|--gtf|--model_manifest|--provenance_mirror_local_prefix)
