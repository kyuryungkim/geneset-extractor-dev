#!/usr/bin/env bash
set -euo pipefail
# Write only the geneset.model.json sidecar for a TCGA methylation model, inside Apptainer.
...
usage(){ echo "Usage: write_tcga_meth_model_json_apptainer.sh --runner methylation_diff [runner args...]"; }
...
case "${RUNNER_NAME}" in
  methylation_diff) RUNNER_REL="NCI_GDC_TCGA_Methylation/src/run_methylation_diff_model.py" ;;
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --run_root|--beta_matrix_tsv|--sample_metadata_tsv|--gtf|--probe_manifest_tsv|--model_manifest)
