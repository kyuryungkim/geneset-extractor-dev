...
SELF_PATH="${REPO_ROOT}/geneset-extractor-dev/run/submit_tcga_rnaseq_models_cluster.sh"
...
JOB_NAME="${TCGA_RNASEQ_JOB_NAME:-tcga_rnaseq_models}"
...
run_task() {
  ...
  local -a cmd
  cmd=("${PYTHON_BIN}" "${REPO_ROOT}/geneset-extractor-dev/NCI_GDC_TCGA_RNAseq/src/build_tcga_rnaseq_genesets.py"
       --tumor_types "${tt}" --models "${model}"
       --counts_tsv "${COUNTS_TSV}" --sample_metadata_tsv "${SAMPLE_METADATA_TSV}"
...
do_submit() {
  ...
  "${QSUB_BIN}" -t "1-${tasks}" -N "${JOB_NAME}" \
    ...
    -v "REPO_ROOT=${REPO_ROOT},DIG_DIR=${DIG_DIR},WORK_ROOT=${WORK_ROOT},PYTHON_BIN=${PYTHON_BIN},TCGA_RNASEQ_OUT_ROOT=${OUT_ROOT},TCGA_RNASEQ_WORKLIST=${WORKLIST},TCGA_RNASEQ_COUNTS_TSV=${COUNTS_TSV},TCGA_RNASEQ_SAMPLE_METADATA_TSV=${SAMPLE_METADATA_TSV},TCGA_RNASEQ_GTF=${GTF},TCGA_TASK_MODE=${MODE},TCGA_TASK_OVERWRITE=${OVERWRITE}" \
    "${SELF_PATH}" --run_task
}
